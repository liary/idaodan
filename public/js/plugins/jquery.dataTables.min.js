...
sEmptyTable:"无数据",sInfo:"共 _TOTAL_ 条",sInfoEmpty:"共 0 条",sInfoFiltered:"",sInfoPostFix:"",sDecimal:"",sThousands:",",sLengthMenu:"Show _MENU_ entries",sLoadingRecords:"<i class='light-blue fa fa-2x fa-spinner fa-spin'></i>",sProcessing:"<i class='light-blue fa fa-2x fa-spinner fa-spin'></i>",sSearch:"Search:",sSearchPlaceholder:"",sUrl:"",sZeroRecords:"无数据！"},oSearch:h.extend({},p.models.oSearch),sAjaxDataProp:"data",sAjaxSource:null,sDom:"lfrtip",searchDelay:null,
...
$(function(){
	$(".table-header .dropdown-menu a").click(function(){
		var $this=$(this);
		var $thisObj=$this.parents(".table-header").find(".dropdown-toggle");
		$thisObj.html($this.text()+' <span class="caret"></span>').attr("data-status",$this.attr("data-status"));
		if($this.attr("data-key")){
			$thisObj.attr("data-key",$this.attr("data-key"));
		}
		var $tableObj=$("#"+$thisObj.attr("data-table"));
		$tableObj.dataTable().api().ajax.reload();
	});
})
!(function(){
	var hash = location.hash,param = hash;
	if(hash.indexOf('#')>-1){
		param = hash.replace('#','');
	}
...
$.extend( true, $.fn.dataTable.defaults, {
	"sDom": "rt<'row'<'col-sm-4 pull-left'i><'col-sm-8 pull-right'p>>",
...
//	            var fnClickHandler = function ( e ) {
//	                e.preventDefault();
//	                if ( oSettings.oApi._fnPageChange(oSettings, e.data.action) && (!oSettings.oInit.noTotal || e.data.action != 'next' )) {
//	                    currentPage--;
//	                    oSettings._iDisplayStart = (currentPage)* oPaging.iLength;
//	                    fnDraw( oSettings );
//	                }else{
//	                	if(e.data.action == 'next' && oSettings.oInit.noTotal){
//		               		currentPage++;
//	                		oSettings._iDisplayStart = (currentPage)* oPaging.iLength;
//		                	fnDraw( oSettings );
//		               	}
...
//          }
            if(oSettings.oInit.noTotal){
	            ...
	            $(nPaging).append(
	            	'<div class="pull-right pagination-input form-inline">跳转到第 <div class="form-group"><input type="text" class="form-control" style="width:50px" /></div> 页 <button class="btn btn-primary" type="button">确定</button></div>'+
	                '<ul class="pagination pull-right">'+
        ...
        "fnUpdate": function ( oSettings, fnDraw ) {
            ...
            var oPaging = oSettings.oInstance.fnPagingInfo();
 			var currentPage=oPaging.iPage+1,
 				totalPage = oPaging.iTotalPages;
 			if(oSettings.oInit.totalUrl){
        		var request = oSettings.oAjaxData;
 				var param = current_dataTable_url.split('?')[1]||"";
 				$.ajax({
	                "type":     'GET',
					"url":      oSettings.oInit.totalUrl+'?'+param,
	                "data":     request,
	                "dataType": "json",
	                "cache":    false,
	                "success":  function ( json ) {
	                	var totalPage = Math.ceil( json.totalCount / oSettings._iDisplayLength ) ;
	                	oSettings._iRecordsTotal = oSettings._iRecordsDisplay=json.totalCount;
	                	_this.setHtml(oSettings, fnDraw ,totalPage );
	                	$(".dataTables_info").html("共"+json.totalCount+"条");
	               }
	            });
 			}else{
				_this.setHtml(oSettings, fnDraw ,totalPage )
        ...
        'setHtml': function(oSettings, fnDraw ,totalPage ){
            var iListLength = 5;
            var oPaging = oSettings.oInstance.fnPagingInfo();
            ...
            var i, j, sClass, iStart, iEnd, iHalf=Math.floor(iListLength/2);
        	 ...
        	 for ( i=0, iLen=an.length ; i<iLen ; i++ ) {
                ...
                $('button', an[i]).unbind("click");
                $('button', an[i]).click(function(e){
                	 e.preventDefault();
	                var page=$(this).parent().find("input").val();
                	if(!page || isNaN(page) || page<1 || page>totalPage){
                		return alert("页码应在1到"+totalPage+"之间");	
                	}
	                oSettings._iDisplayStart = (page-1)* oPaging.iLength;
	                fnDraw( oSettings );
                })
                	...
                	if(currentPage<3){
		                ...
		                if(totalPage-currentPage>2){
		                	$('<li><a href="#">...</a></li>')
		                        ...
		                        .bind('click', function (e) {
		                            e.preventDefault();
		                            oSettings._iDisplayStart = (totalPage-2) * oPaging.iLength;
		                            fnDraw( oSettings );
		                        } );	
		                }
	                }
                	if(currentPage>=3){
                		if(currentPage!=3){
	                		$('<li><a href="#">...</a></li>')
			                        ...
			                        .bind('click', function (e) {
			                            e.preventDefault();
			                            oSettings._iDisplayStart = 1 * oPaging.iLength;
			                            fnDraw( oSettings );
	                        });	
                       }
                        var iEnd=currentPage+1;
                ...
                if ( oPaging.iPage === totalPage-1 || totalPage === 0 ) {
...
$.fn.dataTable.pipeline = function ( opts ) {
    ...
    return function ( request, drawCallback, settings ) {
        ...
        var $this=$(this),$row=$this.parents(".row");
        var $tableHeader = $row.find(".table-header"),$tableFilter = $row.find(".table-filter "),
        	$statusObj = $tableHeader.find(".dropdown-toggle");
        if($statusObj.length>0 && typeof $statusObj.attr("data-status")!='undefined'){
        	var data_key =  $statusObj.attr("data-key") ||"status";
        	request[data_key]=$statusObj.attr("data-status");
        	var exportLink = $tableHeader.find('.exportLink');
        	if(exportLink && exportLink.length>0){
        		setTimeout(function(){
        			var href = exportLink.attr('href');
	        		var _data = {};_data[data_key] = request[data_key];
	        		var param = parseQueryString(href);
	        		param = $.extend(true, param, _data);
	        		exportLink.attr('href',href.split('?')[0]+'?'+querystringify(param));
        		},200);
        	}
        	try{
        		if(JSON.stringify( request[data_key] ) !== JSON.stringify( cacheLastRequest[data_key])){
	        		ajax = true;
	        	}
        	}catch(e){}
        }
        var $dateObj=$tableFilter.find('input[name=date-range-picker]');
        
        if($this.data("searchStatus") && $dateObj.length>0 && $dateObj.val() !=""){
			var dateArray=$dateObj.val().split(" - ");
			request["fromTime"] =moment(dateArray[0]).valueOf()/1000;
			request["toTime"] = moment(dateArray[1]).valueOf()/1000;
        }
		
        if(request.order && request.order.length>0 && request.order[0].dir){
        	request.asc=request.order[0].dir;
        	request.sort=$(this).find("th").eq(request.order[0].column).attr("data-key");
        }
        if ( settings.clearCache ) {
            ...
            ajax = true;
                  ...
                  JSON.stringify( request.status ) !== JSON.stringify( cacheLastRequest.status) ||
				  JSON.stringify( request.fromTime ) !== JSON.stringify( cacheLastRequest.fromTime) ||
				  JSON.stringify( request.toTime ) !== JSON.stringify( cacheLastRequest.toTime) ||
				  opts.url !== cacheLastRequest.url
		) {
            ajax = true;
        }
        cacheLastRequest = $.extend( true, {}, request );
        cacheLastRequest.url = opts.url;
        if ( ajax ) {
            ...
            request.size = requestLength*conf.pages;
            ...
            var formatJson=function(json){
            	json.recordsTotal=json.recordsFiltered=json.totalCount;
            	json.data=json.obj;
            	return json;
            }
            settings.jqXHR = $.ajax( {
                ...
                "cache":    false,
                "success":  function ( json ) {
                	current_dataTable_url = conf.url;
                	var json = formatJson(json);
                    ...
                    if(json && json.success){
                    	if(json.obj && json.obj.length>0){
                    		json.obj.splice( requestLength, json.obj.length );
                    	}else{json.data={}}
                    	try{
                    		drawCallback(json);
                    	}catch(e){
                    		alert(json.message || "表格数据有问题，请重试！");
                    		drawCallback({data:{}});
                    	}
                    	window.mdlTableData = json;
                    }else{
                    	alert(json.message || "加载表格数据失败，请重试！");
                    	json.data={};
                    	drawCallback(json);
                    }
               },
